...
def train_model(X_train,X_test,y_train,y_test):
    ...
    model = nn.Sequential(
        nn.Linear(8, n_units_l0), # 8 Inputs für die 8 Sensorwerte
        nn.ReLU(),
        nn.Linear(n_units_l0, n_units_l1),
        nn.ReLU(),
        nn.Linear(n_units_l1, 3), # drei Outputs für x- und y-Koordinaten des Lasteinleitungspunktes mit der Kraft
      
    ).to(device)  # Modell auf GPU verschieben

    # Definition von Loss- und Optimierungsfunktionen
    loss_fn = nn.MSELoss()  # mean square error
    optimizer = optim.RMSprop(model.parameters(), lr=0.04889279952783688)
