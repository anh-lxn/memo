...
def get_user_inputs():
	...
	y_value_list = []  # Liste zur Speicherung der Y-Positionen
	
	anz_dateien = 121  # Anzahl der Dateien, die verarbeitet werden sollen
	...
	for i in range(anz_dateien // 11):
		y_start = 290  # Startwert für die Y-Positionen
		x_value = x_start - i * abstand
		...
		for j in range(11):
			...
			formatted_id = str(id_start + j + i * 11).zfill(3)  # zfill(3) sorgt dafür, dass die ID immer 3-stellig ist
			...
			y_value = y_start + j * abstand  # Y-Wert wird basierend auf der Schleifenvariable berechnet
			y_value_list.append(y_value)  # Füge den berechneten Y-Wert zur Liste hinzu
	...
	return ids_list, x_value_list, y_value_list, F, anz_dateien  # Gibt die Listen und Werte zurück



# Hauptprogramm Ausführung
if __name__ == "__main__":
	ids_list, x_value_list, y_value_list, F, anz_dateien = get_user_inputs()  # Benutzereingaben abrufen
	for i in range(anz_dateien // 11):
		for j in range (11):
			# Erstellen des Dateipfads basierend auf den oben definierten Parametern
			filepath = f'../../resources/messungen/messung_pi_05_12_15N/{ids_list[j + i * 11]:3}_strain_values_{x_value_list[i]}_{y_value_list[j]}_{F}.csv'
			...
			output_csv_path = f'../../resources/messungen/auswertung/05_12_15N/{ids_list[j + i * 11]:3}_strain_values_{x_value_list[i]}_{y_value_list[j]}_{F}_extracted.csv'
			h_fns.save_strain_values_to_csv(timepoints, strain_data, output_csv_path, dt, x_value_list[i], y_value_list[j], F)
