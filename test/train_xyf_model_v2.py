...
def make_f_features(data: pd.DataFrame) -> pd.DataFrame:
...
def evaluate_models(
    ...
    xy_pred = xy_model.predict(x_xy_test)
    xy_pred = snap_to_known_grid(np.asarray(xy_pred, dtype=float), grid_candidates)
...
def main() -> None:
    ...
    f_train_features = make_f_features(train)
    f_val_features = make_f_features(val)
    f_train_val_features = make_f_features(train_val)
    f_test_features = make_f_features(test)
    ...
    xy_candidates = [
        ...
        (
            ...
            Pipeline(
                [
                    ("regressor", ExtraTreesRegressor(n_estimators=1200, random_state=RANDOM_STATE)),
                ]
            ),
        ),
    ]
    ...
    final_xy_model = best_xy_model
    final_xy_model.fit(xy_train_val_features, y_xy_train_val)

    final_f_model = best_f_model
    final_f_model.fit(f_train_val_features, y_f_train_val)
    ...
    output_dir = ROOT / "test" / "artifacts_v2"
    output_dir.mkdir(parents=True, exist_ok=True)

    joblib.dump(final_xy_model, output_dir / "xy_model.joblib")
    joblib.dump(final_f_model, output_dir / "f_model.joblib")
    ...
    results = {
        ...
        "models": {
            "xy_model": {
                ...
                "features": "normalized + centered sensor features + ring differences + 40mm grid snap",
            },
            "f_model": {
                "type": best_f_name,
                "validation_loss": best_f_val_loss,
                "features": "raw sensor values",
            },
        },
